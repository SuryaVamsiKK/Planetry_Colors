...
public class QuadTree : MonoBehaviour
{
    ...
    void CreateShape()
    {
        ...
        for (int x = 0, i = 0; x <= count; x++)
        {
            ...
            {
                verts[i] = new Vector3((x * (length/count)) + refrence.x, 100f, (z * (width/count)) + refrence.y);  
                verts[i] = (verts[i] + this.transform.position).normalized * 100f;
                //Debug.DrawLine(verts[i],Vector3.zero);            
                i++;
            }
        }
    ...
    void UpdateMesh()
    {
        //if(!avged)
        //{
            avgpoint = CalculateCenter(verts);
            //avged = true;
        //}
        mesh.Clear();
        mesh.vertices = verts;
     ...
     Vector3 CalculateCenter(Vector3[] verts)
   {
       ...
       {
           Vector3 localpoint = this.transform.InverseTransformPoint(verts[i]);
